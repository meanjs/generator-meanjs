...
var TestGenerator = yeoman.generators.NamedBase.extend({
	renderTestFile: function() {
		// Set model names
		this.slugifiedModelName = this._.slugify(this.name);
		...
		// If model file exists we create a test for it otherwise we raise error
		if (fs.existsSync(modelFilePath)) {
			this.template('_tests.js', 'app/tests/' + this.slugifiedPluralModelName + '.js')
		} else {
			throw new Error(this.humanizedModelName + ' model does not exists');
		}
	}
});
