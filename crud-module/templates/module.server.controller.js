...
exports.update = function (req, res) {
  ...
  <%= moduleName %>.title = req.body.title;
  <%= moduleName %>.content = req.body.content;
...
/**
 * List of <%= pluralModuleName %>
 */
exports.list = function (req, res) {
  <%= capitalizedModuleName %>.find().sort('-created').populate('user', 'displayName').exec(function (err, <%= pluralModuleName %>) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
    } else {
      res.json(<%= pluralModuleName %>);
    }
  });
};
